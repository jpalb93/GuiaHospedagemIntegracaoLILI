...
const DEFAULT_CHECKLIST_ITEMS = [
    {
        category: 'Quarto',
        items: ['Lençóis', 'Cobertor', 'Travesseiros', 'Controle Ar', 'Ferro', 'Ventilador'],
    },
    ...
    {
        category: 'Área Serviço',
        items: ['Balde', 'Rodo', 'Varal', 'Vassoura', 'Pregadores', 'Pano de Chão'],
    },
...
const ChecklistSection: React.FC<ChecklistSectionProps> = ({ localSettings, setLocalSettings }) => {
    ...
    const handleImportDefault = () => {
        if (!confirm('Isso irá adicionar vários itens à sua lista atual. Deseja continuar?'))
            return;
    ...
    const grouped = (localSettings.checklist || []).reduce(
        (acc, item) => {
        ...
        <div className="bg-orange-50 dark:bg-orange-900/10 p-4 rounded-xl border border-orange-100 dark:border-orange-800/30">
                            ...
                            {expandedCategories.includes(category) && (
                                <div className="divide-y divide-gray-100 dark:divide-gray-700/50">
                                    {items?.map((item) => (
                                        <div
                                            key={item.id}
                                            className="flex items-center justify-between p-3 pl-9 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"
                                        >
            ...
            <div className="flex gap-2">
                ...
                <datalist id="categories">
                    {Array.from(
                        new Set(localSettings.checklist?.map((i) => i.category || 'Geral') || [])
                    )
                        .sort()
                        .map((cat) => (
            ...
            <div className="mt-3 pt-3 border-t border-orange-200 dark:border-orange-800/30 flex justify-end">
                <button
                    ...
                    className="text-orange-600 dark:text-orange-400 text-xs font-bold hover:underline flex items-center gap-1"
                >
