    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
            <h2 className="text-lg font-bold font-heading mb-4 flex items-center gap-2 text-gray-900 dark:text-white">
                <ImageIcon
                    size={20}
                    className={activeProperty === 'lili' ? 'text-orange-500' : 'text-blue-500'}
                />
                Imagens do Carrossel (
                {activeProperty === 'lili' ? 'Flat da Lili' : 'Flats Integração'})
            </h2>
            ...
            <div className="space-y-4 mb-6">
                {images.map((url, idx) => (
                    <div
                        key={idx}
                        className="flex items-center gap-3 bg-gray-50 dark:bg-gray-900 p-3 rounded-xl border border-gray-100 dark:border-gray-700"
                    >
                ...
                {images.length === 0 && (
                    <p className="text-sm text-gray-400 italic text-center py-4">
                        Nenhuma imagem definida. Usando padrão do sistema.
                    </p>
                )}
            </div>
