...
import { LaundryRecord, Reservation } from '../../types';
...
const LAUNDRY_TYPE_LABELS: Record<string, string> = {
    ...
    dry: 'Secagem',
    wash_and_dry: 'Lavagem e Secagem Completa',
...
const LaundryReport: React.FC<LaundryReportProps> = ({ companyInfo, reservation, laundries }) => {
        ...
        >
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b-2 border-gray-300 dark:border-gray-700 pb-4 sm:pb-6 mb-6 gap-3 print:border-black">
                ...
                <div className="text-left sm:text-right shrink-0">
                    <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white px-4 py-2 rounded-xl text-xs sm:text-sm font-bold inline-block shadow-md uppercase tracking-wider font-heading print:bg-stone-900 print:text-white">
                        RELATÓRIO DE LAVANDERIA / ROUPAS
                    </div>
            ...
            {/* DESCRIPTION */}
            <div className="bg-blue-50/60 dark:bg-blue-950/20 border border-blue-200/60 dark:border-blue-900/30 p-4 rounded-xl mb-6 print:bg-gray-100 print:border-gray-300">
                <p className="text-xs sm:text-sm text-stone-700 dark:text-gray-300 leading-relaxed font-medium">
                    Este documento detalha o histórico de serviços e ciclos de{' '}
                    <strong>Lavagem de Roupa</strong> solicitados/executados como{' '}
                    <strong>Custos Adicionais</strong> durante a estadia.
                </p>
            </div>
            ...
            <div className="mb-6">
                <h3 className="text-sm font-bold uppercase tracking-wider text-gray-900 dark:text-white mb-3 flex items-center gap-2 font-heading">
                    ...
                    Histórico de Ciclos de Lavanderia ({laundries.length} registros · {totalCycles}{' '}
                    ciclos)
                </h3>
                ...
                {laundries.length === 0 ? (
                    <div className="p-6 text-center text-sm text-gray-500 bg-gray-50 dark:bg-gray-800/40 rounded-xl border border-dashed border-gray-300 dark:border-gray-700">
                        Nenhum registro de lavagem de roupa para esta reserva.
                    </div>
                ) : (
                        ...
                        <table className="w-full text-left border-collapse text-xs">
                            <thead className="bg-gray-100 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 print:bg-gray-200">
                                <tr>
                                    ...
                                    <th className="py-2.5 px-3 font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider text-center">
                                        Ciclos
                                    </th>
                                    <th className="py-2.5 px-3 font-bold text-gray-700 dark:text-gray-300 uppercase tracking-wider">
                                        Pagamento
                            ...
                            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                {laundries.map((l) => {
                                    const typeLabel =
                                        l.typeLabel || LAUNDRY_TYPE_LABELS[l.type] || l.type;
                                    const payInfo = PAYMENT_STATUS_LABELS[l.paymentStatus] || {
                                        label: l.paymentStatus,
                                        color: 'bg-gray-100 text-gray-800',
                                    };
                                    const formattedDate = l.date
                                        ? new Date(l.date + 'T00:00:00').toLocaleDateString('pt-BR')
                                        : '—';
                                    const cycles = l.cyclesCount || 1;
            ...
            <div className="bg-stone-900 text-white dark:bg-gray-800 p-4 rounded-xl flex items-center justify-between gap-4 mb-8 shadow-md print:bg-stone-900 print:text-white">
                <div>
                    <p className="text-xs uppercase tracking-widest text-blue-400 font-bold font-heading">
                        Total em Custos Adicionais (Lavanderia)
                    </p>
                    <p className="text-[11px] text-stone-300 mt-0.5">
                        {totalCycles} ciclo(s) em {laundries.length} registro(s) para esta reserva
                    </p>
                </div>
