...
const SuggestionsManager: React.FC<SuggestionsManagerProps> = ({ suggestions }) => {
    const [localSuggestions, setLocalSuggestions] = useState<SmartSuggestionsConfig>(
        suggestions.data
    );
    ...
    const addSuggestion = (category: keyof SmartSuggestionsConfig) => {
        if (
            tempSuggestion.category === category &&
            tempSuggestion.title &&
            tempSuggestion.description
        ) {
    ...
    const renderSection = (
        ...
        return (
            <div
                className={`p-4 rounded-xl border ${colorClass} bg-opacity-10 bg-white dark:bg-opacity-5`}
            >
                ...
                <div className="space-y-2 mb-4">
                    {items.length === 0 && (
                        <p className="text-xs text-gray-400 italic">Nenhuma sugestão cadastrada.</p>
                    )}
                    {items.map((item) => (
                        <div
                            key={item.id}
                            className="flex justify-between items-start bg-white dark:bg-gray-800 p-3 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm"
                        >
                            <div>
                    ...
                    <textarea
                        ...
                        onChange={(e) =>
                            setTempSuggestion({
                                ...
                                title: tempSuggestion.category === key ? tempSuggestion.title : '',
                            })
                        }
                    />
                    <button
        ...
        <div className="space-y-6 max-w-4xl mx-auto">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
                    Adicione várias sugestões para cada horário. O sistema escolherá{' '}
                    <strong>uma aleatoriamente</strong> para mostrar ao hóspede a cada acesso.
                </p>
