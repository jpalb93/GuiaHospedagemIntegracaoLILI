...
const GuestHeader: React.FC<GuestHeaderProps> = ({
    ...
    const activeNoticeText = globalNotice?.active ? globalNotice.text : (appSettings?.noticeActive ? appSettings.noticeText : '');
    const isNoticeActive = globalNotice?.active || appSettings?.noticeActive;

    const showGlobalBanner = isNoticeActive && activeNoticeText && activeNoticeText !== dismissedAlerts.global;
    const showPersonalBanner = config.guestAlertActive && config.guestAlertText !== dismissedAlerts.personal;
        ...
        <div className="sticky top-0 left-0 w-full z-[45] flex flex-col shadow-md">
            {showGlobalBanner && (
                <div className="bg-yellow-500 text-white animate-fadeIn relative">
                    ...
                    <button onClick={() => onDismissAlert('global', activeNoticeText || '')} className="absolute top-1 right-1 p-1.5 hover:bg-white/20 rounded-full transition-colors"><X size={16} /></button>
                </div>
            )}
            {showPersonalBanner && (
                <div className="bg-blue-600 text-white animate-fadeIn border-t border-blue-500/50 relative">
                    ...
                    <button onClick={() => onDismissAlert('personal', config.guestAlertText || '')} className="absolute top-1 right-1 p-1.5 hover:bg-white/20 rounded-full transition-colors"><X size={16} /></button>
                </div>
            )}
        </div>
    );
