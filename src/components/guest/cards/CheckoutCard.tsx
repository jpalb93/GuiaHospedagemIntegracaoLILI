...
const CheckoutCard: React.FC<CheckoutCardProps> = ({
    ...
    const onOpenDriverMode = () => {
        window.open(`https://www.google.com/maps/search/?api=1&query=R.+São+José,+475B,+Centro,+Petrolina+-+PE`, '_blank');
    };

    return (
        <div className={`flex flex-col h-full ${theme.background} p-4 rounded-3xl text-white shadow-2xl border border-red-500/20 relative overflow-hidden w-full animate-gold-pulse`}>
            ...
            <div className="mb-3 relative z-10 text-center">
                <h3 className={`text-lg font-heading font-bold ${theme.text.primary} mb-1`}>Volte sempre, viu? 👋</h3>
            </div>
            ...
            {/* SUPPORT BUTTON */}
            <button onClick={() => { if (navigator.vibrate) navigator.vibrate(50); onOpenSupport(); }} className={`w-full py-3 ${theme.button.primary} text-white text-[11px] font-bold uppercase tracking-wide transition-all flex items-center justify-center gap-2 rounded-xl shadow-lg active:scale-[0.98] relative z-10 mb-4`}>
            ...
            <div className="flex items-center justify-center gap-2 text-red-100/60 relative z-10 mt-auto">
                ...
                <p className={`text-sm font-mono font-bold ${theme.text.primary} bg-red-500/10 px-2 py-0.5 rounded border border-red-500/20`}>
