...
const SectionCard: React.FC<SectionCardProps> = ({
    ...
    return (
        <div
            className={`bg-white dark:bg-gray-800 rounded-[24px] shadow-[0_2px_8px_rgba(0,0,0,0.04)] dark:shadow-none border border-gray-100 dark:border-gray-700 break-inside-avoid transition-all duration-300 hover:shadow-[0_6px_16px_rgba(0,0,0,0.06)] relative ${isTrigger ? 'cursor-pointer active:scale-[0.98]' : ''}`}
        >
            <button
            ...
            >
                <div className="flex items-center gap-4">
                    <div
                        className={`p-3 rounded-xl ${color} text-white shadow-md shadow-gray-200 dark:shadow-none group-hover:scale-105 transition-transform duration-300`}
                    >
                        <Icon size={20} strokeWidth={2.5} />
                    </div>
                    <span className="text-base sm:text-lg font-heading font-bold text-gray-900 dark:text-white group-hover:text-gray-700 dark:group-hover:text-gray-300 transition-colors">
                        {title}
                    </span>
                </div>
