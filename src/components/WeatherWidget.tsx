...
const WeatherWidget: React.FC = () => {
  const [weather, setWeather] = useState<{ temp: number; code: number } | null>({ temp: 30, code: 0 });

  useEffect(() => {
    const fetchWeather = async () => {
      try {
        const res = await fetch('https://api.open-meteo.com/v1/forecast?latitude=-9.39&longitude=-40.50&current=temperature_2m,weather_code&timezone=America%2FSao_Paulo');
  ...
  return (
    <div className={`px-3 py-1.5 rounded-full text-[10px] sm:text-xs font-bold font-sans shadow-lg border flex items-center gap-1.5 backdrop-blur-md transition-all animate-fadeIn
    ...
    >
      {info.alert ? <ThermometerSun size={12} className="text-yellow-300 animate-breathing" /> : <CloudSun size={12} className="animate-breathing" />}
      {info.text}
    </div>
  );
};
