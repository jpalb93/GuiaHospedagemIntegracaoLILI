...
export const handleApiError = (res: VercelResponse, error: unknown): VercelResponse => {
    ...
    if (error instanceof ApiError) {
        logger.warn(`API Error [${error.code || 'UNKNOWN'}]:`, { message: error.message });
...
export const ApiErrors = {
    MissingParameter: (param: string) =>
        new ApiError(400, `Missing required parameter: ${param}`, 'MISSING_PARAMETER'),
    InvalidParameter: (param: string) =>
        new ApiError(400, `Invalid parameter: ${param}`, 'INVALID_PARAMETER'),
